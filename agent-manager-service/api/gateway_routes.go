...
func RegisterGatewayRoutes(mux *http.ServeMux, ctrl controllers.GatewayController) {
	...
	middleware.HandleFuncWithValidation(mux, "POST /orgs/{orgName}/gateways/{gatewayID}/environments/{envName}", ctrl.AssignGatewayToEnvironment)
	middleware.HandleFuncWithValidation(mux, "DELETE /orgs/{orgName}/gateways/{gatewayID}/environments/{envName}", ctrl.RemoveGatewayFromEnvironment)
